...
/**
 * An extension <code>AbstractBiotaber</code>, which adds clinical specific code to the "worker" class (the main unit of work for a thread). 
 * It will generate clinical biotab files for a given study.  
 * <p>
 * However, there was never any really intention to have a completely well separated application that handles both clinical and biospecimen biotabs. 
 * Almost all of the code in this entire application is very specific to clinical data. 
 * </p> 
 * 
 * @author John Deardurff 
 */
public class ClinicalBiotaber extends AbstractBiotaber {
	
	protected boolean writeBiotabs = true;
	
	public ClinicalBiotaber(Properties properties, String diseaseCode) throws BiotabInitializationException, XsdVersionFileException {
		super(properties, BaseBiotaberApp.XmlFileType.CLINICAL, diseaseCode);
		init();
	}
	
	public ClinicalBiotaber(Properties properties, String diseaseCode, boolean writeBiotabs) throws BiotabInitializationException, XsdVersionFileException {
	...
	public void run() {
		try {
			...
			log.info("--- Clinical Biotab Generation (Creating Headers for study " + this.getJobDTO().getDiseaseCode() + ")...");
			loadHeadersForStudy();
			long endTime = System.currentTimeMillis();
			long duration = endTime - startTime;
			log.info(MessageFormat.format("--- Clinical Biotab Generation (Header Creation Complete) for study ({0}) (RUNTIME: {1,number,#.###} ) minutes:", 
			...
			log.info("--- Clinical Biotab Generation (Creating Tables for study + " + this.getJobDTO().getDiseaseCode() + ")...");
			createTables();
			endTime = System.currentTimeMillis();
			duration = endTime - startTime;
			log.info(MessageFormat.format("--- Clinical Biotab Generation (Table Creation Complete) for study ({0}) (RUNTIME: {1,number,#.###} ) minutes:", 
					this.getJobDTO().getDiseaseCode(), ((float)duration/1000.0)/60.0 ));
			
			
		} catch (BiotabHeaderCreationException e) {
			log.fatal("Clinical Biotab Generation encountered errors creating headers for one of the biotab files.", e);
		}  catch (BiotabTableCreationException e) {
			log.fatal("Clinical Biotab Generation encountered errors populating one of the tables for the biotab files.", e);
		}
	...
	protected void loadHeadersForStudy() throws BiotabHeaderCreationException {	
		...
		try {
			...
			throw new BiotabHeaderCreationException("Sample Clinical XML file could not be read", e);
		} 
