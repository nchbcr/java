 ...
 */
public class OmfBiotaberApp extends BaseBiotaberApp {
	...
	protected void start(String[] args) {
		try {
			...
			log.info("OMF Biotab Generation Started...");
			for (String study : studies) {
				try {
					OmfBiotaber biotaber = new OmfBiotaber(System.getProperties(), study.toUpperCase());
					...
					log.error("Could not start OMF biotaber for " + study.toUpperCase(), e);
				}
			}	
			...
			log.info(MessageFormat.format("OMF Biotab Generation Completed (RUNTIME: {0,number,#.###} ) minutes.", ((float)duration/1000.0)/60.0 ));
			
			
		} catch (BiotabInitializationException e) {
			log.fatal("OMF Biotab Generation could not be initialized.", e);
		} catch (XsdVersionFileException e) {
			log.fatal("OMF Biotab Generation could not read the xsd version file. No biotab files were generated.", e);
		} catch (Throwable e) {
			log.fatal("OMF Biotab Generation failed due to an unknown error. ", e);
		}
	}
