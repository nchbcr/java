 ...
 */
public class PpsBiotaberApp extends BaseBiotaberApp {
	...
	protected void start(String[] args) {
		try {
			...
			log.info("Pathology Prescreen Biotab Generation Started...");
			for (String study : studies) {
				try {
					PpsBiotaber biotaber = new PpsBiotaber(System.getProperties(), study.toUpperCase());
					...
					log.error("Could not start Pathology Prescreen biotaber for " + study.toUpperCase(), e);
				}
			}	
			...
			log.info(MessageFormat.format("Pathology Prescreen Biotab Generation Completed (RUNTIME: {0,number,#.###} ) minutes.", ((float)duration/1000.0)/60.0 ));
			
			
		} catch (BiotabInitializationException e) {
			log.fatal("Pathology Prescreen Biotab Generation could not be initialized.", e);
		} catch (XsdVersionFileException e) {
			log.fatal("Pathology Prescreen Biotab Generation could not read the xsd version file. No biotab files were generated.", e);
		} catch (Throwable e) {
			log.fatal("Pathology Prescreen Biotab Generation failed due to an unknown error. ", e);
		}
	}
	
	public static void main(String[] args) {
		PpsBiotaberApp ppsApp = new PpsBiotaberApp();
		ppsApp.start(args);
	}
}