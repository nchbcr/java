...
public class SsfBiotaber extends AbstractBiotaber {
	
	protected boolean writeBiotabs = true;
	
	public SsfBiotaber(Properties properties, String diseaseCode) throws BiotabInitializationException, XsdVersionFileException {
		super(properties, BaseBiotaberApp.XmlFileType.SSF, diseaseCode);
		init();
	}
	
	public SsfBiotaber(Properties properties, String diseaseCode, boolean writeBiotabs) throws BiotabInitializationException, XsdVersionFileException {
	...
	public void run() {
		try {
			...
			log.info("--- SSF Biotab Generation (Creating Headers for study " + this.getJobDTO().getDiseaseCode() + ")...");
			loadHeadersForStudy();
			long endTime = System.currentTimeMillis();
			long duration = endTime - startTime;
			log.info(MessageFormat.format("--- SSF Biotab Generation (Header Creation Complete) for study ({0}) (RUNTIME: {1,number,#.###} ) minutes:", 
			...
			log.info("--- SSF Biotab Generation (Creating Tables for study + " + this.getJobDTO().getDiseaseCode() + ")...");
			createTables();
			endTime = System.currentTimeMillis();
			duration = endTime - startTime;
			log.info(MessageFormat.format("--- SSF Biotab Generation (Table Creation Complete) for study ({0}) (RUNTIME: {1,number,#.###} ) minutes:", 
					this.getJobDTO().getDiseaseCode(), ((float)duration/1000.0)/60.0 ));
			
			
		} catch (BiotabHeaderCreationException e) {
			log.fatal("SSF Biotab Generation encountered errors creating headers for one of the biotab files.", e);
		}  catch (BiotabTableCreationException e) {
			log.fatal("SSF Biotab Generation encountered errors populating one of the tables for the biotab files.", e);
		}
	...
	protected void loadHeadersForStudy() throws BiotabHeaderCreationException {	
		...
		try {
			...
			throw new BiotabHeaderCreationException("Sample SSF XML file for XML could not be read", e);
		} 
